...
var CORE = {
    ...
    init: function(){
        //Load all scripts
        for(var f in this.modules) if (this.modules[f].preInit)  this.modules[f].preInit();  //To load independent things that may be used by other components
        for(var f in this.modules) if (this.modules[f].init)     this.modules[f].init();     //Init
        for(var f in this.modules) if (this.modules[f].postInit) this.modules[f].postInit(); //If you specifically wait for some module to be loaded
    },
    ...
    registerModule: function( module , name = null){
        ...
        switch(module.constructor.name){
            case "Object":      if(module.name && !name) name = module.name;        
                                instance = module;                                  break;
            case "Function":    if(!name) name = module.name;
                                instance = new module();                            break;
            default:            console.error( "[1] Invalid module type", module );
        }
